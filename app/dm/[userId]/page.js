...
export default function DMPage() {
  ...
  useEffect(() => {
    if (bottomRef.current && messagesRef.current) {
  ...
  async function loadData() {
    ...
    const { data: otherUser } = await supabase.from('profiles').select('id, username, accent_color, last_seen, status_public, tier').eq('id', userId).single()
    setOther(otherUser)
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        borderBottom: `1px solid ${accent}22`, background: '#111'
      }}>
      ...
      <div ref={messagesRef} style={{ flex: 1, overflowY: 'auto', padding: '16px', WebkitOverflowScrolling: 'touch' }}>
        {messages.length === 0 && (
          <p style={{ color: '#333', fontSize: 14, textAlign: 'center', marginTop: 40 }}>Start a conversation with {other?.username}</p>
        )}
        {messages.map(msg => {
